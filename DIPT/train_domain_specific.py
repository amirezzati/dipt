...
from prompts_utils import train_with_domain_prompt,zero_shot_classification
...
def main():
    ...
    parser.add_argument("--val_ratio", type=float, default=0.1, help="Fraction of the dataset to use for validation.")
    ...
    print(domains)
    datasets = MultipleDomainDataset(root_dir, domains, transform=transform)
    dataloaders = DataLoader(datasets, batch_size=args.batch_size, shuffle=True)
    ...
    train_with_domain_prompt(
        ...
        eval_interval=args.eval_interval  # Pass eval_interval here
    )
