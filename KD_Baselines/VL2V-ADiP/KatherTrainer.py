...
root_data_dir = args['data_dir'] + '/kather100k/train'

# print(root_dir)
# train_dataset = MultipleDomainDataset(root_dir=root_data_dir, domains=['0', '1', '2', '3', '4'], transform=transform)
# val_dataset = MultipleDomainDataset(root_dir=root_data_dir, domains=['5'], transform=transform)
# test_dataset = MultipleDomainDataset(root_dir=root_data_dir, domains=['6'],  # Specify domains to include
#     transform=transform
# )
...
data = {}
data['name'] = "kather100k"
data['num_classes'] = 9
data['num_domains'] = 7
data['class_names'] = ['Adipose', 'background', 'debris', 'lymphocytes', 'mucus', 'smooth muscle', 'normal colon mucosa',
                       'cancer-associated stroma', 'colorectal adenocarcinoma epithelium']
data['input_shape'] = (3, 224, 224)
...
def train_all_plip(args, data, train_doms, val_doms, transform, batch_size, checkpoint_freq, logger=None):
    ...
    # saving args to config file
    output_dir = args["out_dir"]
    file_path = output_dir / 'config.txt'
    ...
    for _index, (train_domains, val_domain) in enumerate(zip(train_doms, val_doms)):
        ...
        for i_stage in range(0, 2):  # changed this
            ...
            print(f"Stage {stage} of training_dom:{train_domains} and val_dom:{val_domain} completed.\n")
...
def inference_all_plip(args, data, train_doms, val_doms, test_dom, transform, batch_size, logger=None, last = False):
    ...
    # saving args to config file
    output_dir = args["out_dir"]
    file_path = output_dir / 'inf_config.txt'
    with open(file_path, "w") as file:
        file.write('data:\n')
        for key, value in data.items():
            file.write(f"{key}: {value}\n")
...
args['name'] = "PLIP_kather100k_BL_ViT_2e-5"
args['backbone'] = "ViT-B/16"    # "ViT-L/14" "vit-B/16"
args['model'] = "vit-base"

# args['name'] = "PLIP_kather100k_BL_RN"
# args['backbone'] = "RN50"    #\ resnet 50
# args['model'] = "resnet50"
...
# train_all_plip(args, data, train_doms, val_doms, transform, batch_size, checkpoint_freq = 300, logger=logger)
