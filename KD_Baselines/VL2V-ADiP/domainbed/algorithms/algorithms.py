...
class ERM_LLT(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_LLT, self).__init__(input_shape, num_classes, num_domains, hparams)
        print("Linear layer training ....")
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.optimizer = get_optimizer(
...
class ERM_PRJ(Algorithm):
    """
    Empirical Risk Minimization (ERM) with projection
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_PRJ, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.embed_dim = 512
        print("\n\nERM with projection ... \n\n")
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.projection = nn.Linear(self.featurizer.n_outputs, self.embed_dim)
        self.classifier = nn.Linear(self.embed_dim, num_classes)
        self.network = nn.Sequential(self.featurizer, self.projection, self.classifier)
        self.optimizer = get_optimizer(
            hparams["optimizer"],
            self.network.parameters(),
            lr=self.hparams["lr"],
