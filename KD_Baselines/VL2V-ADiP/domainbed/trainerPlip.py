...
def train(
    ...
    print('open target is: ======================', open_target)
    ...
    batch_sizes = np.full([n_envs], hparams["batch_size"], dtype=np.int64) # changed int to int64
    ...
    if "DFC" in args.algorithm:
        print('before initializing Plip')
        # clip_model = algorithms.CLIP(hparams)
        plip_model = algorithms.PLIP(hparams)
        print('after plip before alg')
    ...
    if hparams["pretrained"] is not None and hparams["pretrained"] not in ["True", "False"]:
        ...
        if "stage1" in hparams["pretrained"]:
            ...
            # model_path = hparams["pretrained"] + "TE{}_best.pth".format(test_envs[0]) # changed to this:
            model_path = hparams["pretrained_path"] + hparams["pretrained"] + "TE{}_best.pth".format(test_envs[0]) 
            
            model_dict = torch.load(model_path)
            algorithm_dict = model_dict["model_dict"]
            algorithm.load_state_dict(algorithm_dict, strict=True)

        # load stage 2 model for stage 3 [MIRO+SWAD] -> we use this for inference
        # elif stage == "stage2":
        elif "stage2" in hparams["pretrained"]:
            logger.info("Loading Stage 2 pre-trained model ...")
            if args.open_set:
                # model_path = hparams["pretrained"] + "TE{}_best.pth".format(args.model_load) # changed this to bellow: 
                # model_path = hparams["pretrain_pth"] + "TE{}_best.pth".format(test_envs[0]) # changed to bellow: 
                 model_path = hparams["pretrained_path"] + hparams["pretrained"]  + "TE{}_best.pth".format(test_envs[0])
            else:
            ...
            if hparams["cls_pth"] != "":
                ...
                model_path_cls = hparams["cls_pth"] + "TE{}_best.pth".format(test_envs[0]) # changed this to bellow: not for now 
                # model_path_cls = hparams["pretrain_pth"] + hparams["cls_pth"] + "TE{}_best.pth".format(test_envs[0])
    ...
    for step in tqdm(range(n_steps), desc=f"Training", bar_format="{l_bar}{bar:20}{r_bar}"):
        ...
        # if "DFC" in args.algorithm and args.algorithm.split("_")[1] != "STAGE3":
        if "DFC" in args.algorithm:
            step_vals = algorithm.update(**inputs, clip_model=plip_model)
        else:
        ...
        if step % checkpoint_freq == 0:
            ...
            # ):
                ...
                # ############################### save file name ###############################################
                filename = hparams["stage"] + "TE{}_best.pth".format(test_env_str)
