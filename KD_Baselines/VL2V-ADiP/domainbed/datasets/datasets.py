...
class MultipleEnvironmentImageFolder(MultipleDomainDataset):
    def __init__(self, root, open_target=None):
        super().__init__()
        environments = [f.name for f in os.scandir(root) if f.is_dir()]
        environments = [
            i for i in environments if "Snow" not in i and "Cartoon" not in i
        ]
        environments = sorted(environments)
        self.environments = environments

        self.datasets = []
        print("MultipleDomainDataset print =======================")
        ...
        self.class_names = self.datasets[-1].classes
        print(self.class_names)
        
        # # [#] open-set modifications
        if open_target:
            in_class = [i for i in range(open_target[1])]
            out_class = [i for i in range(open_target[1], self.num_classes)]
            for env_idx in range(len(environments)):
                env_data = self.datasets[env_idx]
                if env_idx == open_target[0]:
                    idx = [i for i in range(len(env_data)) if env_data.imgs[i][1] in out_class]
                else:
                    idx = [i for i in range(len(env_data)) if env_data.imgs[i][1] in in_class]
                env_dataset = Subset(env_data, idx)
                self.datasets[env_idx] = env_dataset
        
class MultipleEnvironmentImageFolderOST(MultipleDomainDataset):
    def __init__(self, root, open_target=None):
        super().__init__()
        environments = [f.name for f in os.scandir(root) if f.is_dir()]
        environments = [
...
class OfficeHome(MultipleEnvironmentImageFolder):
    ...
    def __init__(self, root, open_target=None):
        ...
        super().__init__(self.dir, open_target=open_target)

class OfficeHomeOST(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["A", "C", "P", "R"]
...
class Camelyon17(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["A", "C", "P", "R"]
