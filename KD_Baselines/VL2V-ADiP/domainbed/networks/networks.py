...
class ResNet(torch.nn.Module):
    ...
    def __init__(self, input_shape, hparams):
        ...
        self.network, self.n_outputs = get_backbone(
            hparams.model, preserve_readout=False, pretrained=hparams.pretrained
        )
...
# class DeiT_Base(timm.models.vit_deit_base_distilled_patch16_224):
#     def __init__(self):
#         super(DeiT_Base, self).__init__(pretrained=True)
...
class VIT(torch.nn.Module):
    ...
    def __init__(self, input_shape, hparams):
        ...
        if hparams["pretrained"] == "False":
            self.network, self.n_outputs = get_backbone(
                hparams.backbone, preserve_readout=False, pretrained=False
            )
        else:
            self.network, self.n_outputs = get_backbone(
                hparams.backbone, preserve_readout=False, pretrained=True
            )
        self.hparams = hparams
...
def Featurizer(input_shape, hparams, **kwargs):
    ...
    if (
        hparams["backbone"].startswith("deit")
        or hparams["backbone"].startswith("vit")
        or hparams["backbone"].startswith("clip")
    ):
        hparams["model"] = hparams["backbone"]

    # if input_shape[1:3] == (224, 224):
    if hparams["backbone"].startswith("vit") or hparams["backbone"].startswith("deit"):
        print("I'm returning vit backbone")
    ...
    # elif hparams["backbone"].startswith("deit") or hparams["backbone"].startswith(
    #     "vit"
    # ):
    #     print("USING ViT backbone  .....")
    #     return VIT(input_shape, hparams)
    # elif input_shape[1:3] == (224, 224):
    #     if hparams["backbone"].startswith("clip"):
    #         print("USING CLIP INIT .....")
    #         hparams["model"] = hparams["backbone"]
