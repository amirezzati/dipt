...
def get_backbone(name, preserve_readout, pretrained):
    if not pretrained:
        assert name in [
            ...
            "swag_regnety_16gf",
            "vit-base",
        ], "Only RN50/RegNet supports non-pretrained network"
    ...
    # [#] VIT models
        ...
        try:
            network = timm.models.vit_deit_base_distilled_patch16_224(pretrained=True)
        except:
            network = timm.models.deit_base_distilled_patch16_224(pretrained=True)
        network.head = nn.Identity()
        network.head_dist = nn.Identity()
        n_outputs = 768
    elif name == "deit-small":
        network = timm.models.deit_small_distilled_patch16_224(pretrained=True)
        network.head = nn.Identity()
        network.head_dist = nn.Identity()
        n_outputs = 384
    elif name == "vit-base":
        network = timm.models.vit_base_patch16_224(pretrained=True)
        network.head = nn.Identity()
        n_outputs = 768
    elif name == "vit-small":
        print("USING ViT-S/16")
        network = timm.models.vit_small_patch16_224(pretrained=True)
        network.head = nn.Identity()
        n_outputs = 384

    ## what about vit large????
    elif name == "swag_regnety_16gf":
        # No readout layer as default
        network = torchhub_load(
            "facebookresearch/swag", model="regnety_16gf", pretrained=pretrained
        )
