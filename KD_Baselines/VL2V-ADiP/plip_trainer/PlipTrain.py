...
import dg.Prompts.load_prompts as pt ## loading prompts
# import PlipTrain
...
def load_algorithm(stage, algorithm, args, data, logger, last = False):
    ...
    if stage == 1:
        ...
        if not last:
            ...
            print(f"Loading from fallback path: {last_model_path}")
            model_dict = torch.load(last_model_path)
...
def train(stage, train_loader, val_loader, args, data, checkpoint_freq, logger = None):
    ...
    if args["Prompt"]:
        ...
        learned_prompts = pt.get_learned_promtps(num_tokens, data['train_doms'] )
